...
# from services.book_service import get_books, get_book_details

# # Set page configuration
# st.set_page_config(page_title="Book Catalog", page_icon="📚", layout="wide")

# # Sidebar Navigation
# st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/6/6b/Goodreads_logo.png", width=180)
# st.sidebar.title("📚 Book Catalog")
# page = st.sidebar.radio("Navigation", ["Home", "Search", "Recommendations", "Wishlist", "Borrowed Books", "Analytics"])
...
#             st.caption(f"⭐ {book['rating']} | {book['author']}")
#             if st.button(f"More about {book['title']}", key=book["title"]):
...
#     else:
#         display_books(st.session_state.wishlist)
...
#     st.title("📚 Borrowed Books")
#     if not st.session_state.borrowed:
#         st.write("No books borrowed yet.")
#     else:
#         display_books(st.session_state.borrowed)
...
#             if details not in st.session_state.wishlist:
#                 st.session_state.wishlist.append(details)
#                 st.success("Added to Wishlist!")
#     with col2:
#         if st.button("Borrow Book"):
#             if details not in st.session_state.borrowed:
#                 st.session_state.borrowed.append(details)
...
# # Recommendations
# from services.book_service import get_recommendations
# if page == "Recommendations" and "selected_book" in st.session_state:
...
#     genre_count = pd.Series([genre for book in books for genre in book['genres']]).value_counts()
...
# #         st.rerun()
# # else:
# #     with st.sidebar.expander("🔑 Login / Signup", expanded=False):
# #         username = st.text_input("Username")
# #         password = st.text_input("Password", type="password")
# #         if st.button("Login"):
# #             if username and password:
...
# #     return [book for book in books if query in book.get('title', '').lower() or query in book.get('author', '').lower()]
...
# #             display_books(results)
# #         else:
# #             st.warning("No results found. Please try a different title or author.")
...
# #             st.write("No recommendations available.")
# #     else:
# #         st.warning("Select a book from 'Home' to see recommendations.")
